...
デザインはプロジェクト直下にある`DESIGN.md`を参照してください。
...
vp update        # パッケージ更新
vpx skills experimental_install # skillsのインストール
```
...
このプロジェクトは **TDD（テスト駆動開発）** で開発する。

1. **テスト項目を提示** → レビュー・承認待ち
2. **テストコード作成** → 失敗確認（Red）
3. **実装** → テストパス確認（Green）
4. **リファクタリング** → テストが引き続きパス（Refactor）

- テストには `vite-plus/test` からインポートした Vitest を使用する
